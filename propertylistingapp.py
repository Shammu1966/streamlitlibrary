...
if (add_selectbox == "Deal with Customer Code"):
  ...
  def edit_action(row_id):
        st.session_state["edit_id"] = row_id
        with col2:
                st.info(f"Edit clicked for customer: {row['Code']}")

                # Example: Show inputs to edit values (you can expand this)
                new_name = st.text_input("New Customer Name", row['Customer Name'], key=f"name_{index}")
                new_contact = st.text_input("New Contact Person", row['Contact Person'], key=f"contact_{index}")
                if st.button("Submit Changes", key=f"submit_{index}"):
                    # Add your SQL UPDATE logic here
                    st.success(f"Updated customer: {row['Code']}")
  def delete_action(row_id):
              st.session_state["delete_id"] = row_id
              with col2:
                st.warning(f"Delete clicked for customer: {row['Code']}")
                # Add your SQL DELETE logic here
                # Example:
                # cursor = conn.cursor()
  ...
  def init_connection():
    ...
    ##st.dataframe(df.head(4), height=200)
     ...
     df.columns = ['Code', 'Customer Name', 'Contact Person']  # Rename for clarity
    
     st.subheader("Add Modify Delete Customer List")
     with st.container():
      col1, col2 = st.columns(2)
      with col1:
        # Loop through rows and show edit/delete buttons
       for index, row in df.iterrows():
            col1, col2, col3, col4, col5 = st.columns([2, 3, 3, 2, 2])
            col1.write(row['Code'])
            col2.write(row['Customer Name'])
            col3.write(row['Contact Person'])

            if col4.button("Edit", key=f"edit_{index}"):
                edit_action(index) ## edit_action(row["{index}"])
            if col5.button("Delete", key=f"delete_{index}"):
               delete_action(index) ##  delete_action(row["{index}"])
